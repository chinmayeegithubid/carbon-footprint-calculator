...
from chatbot import chatbot_ui, generate_personalized_tips
from functions import chart
...
##with open("./style/main.md", "r", encoding="utf-8") as main_page:
   ## st.markdown(main_page.read(), unsafe_allow_html=True)  # ✅ Correct one

##_,but,_ = main.columns([1,2,1])
##if but.button("Calculate Your Carbon Footprint!", type="primary"):
   ## click_element('tab-1')
# ✅ First show the welcome + intro content
with open("./style/main.md", "r", encoding="utf-8") as main_page:
    main.markdown(main_page.read(), unsafe_allow_html=True)

# ✅ Then show the Calculate button AFTER content
_, but, _ = main.columns([1, 2, 1])
if but.button("🚀 Calculate Your Carbon Footprint!", type="primary", help="Click to begin"):
   click_element('tab-1')
...
# Estimate emissions for Gemini AI-based suggestions
transport_emission = df["Vehicle Monthly Distance Km"][0] * 0.21 * 12
electricity_emission = df["How Long TV PC Daily Hour"][0] * 0.4 * 365
diet_emission = {"omnivore": 2.5, "pescatarian": 1.9, "vegetarian": 1.7, "vegan": 1.5}.get(df["Diet"][0], 2.0) * 365
waste_emission = df["Waste Bag Weekly Count"][0] * 4.34 * 0.5 * 12
...
# 👇 Layout: Show Results button in center column
_, resultbutton, _ = tab5.columns([1, 1, 1])  
...
# 👇 When user clicks "Show Results"
if resultbutton.button("✅ Show Results", type="primary"):
    st.session_state["show_results"] = True

# 👇 Conditional rendering of results
if st.session_state["show_results"]:
    tab5.success("✅ Show Results clicked!")

    # 🔧 Generate chart image
    chart_image = chart(model, ss, sample_df, prediction)

    # 📊 Show chart
    tab5.image(chart_image, use_container_width=True)

    # 🌳 Calculate tree count
    tree_count = round(prediction / 411.4)
    tab5.markdown("Want to make a real-world impact? Visit TEMA to learn more about tree planting and reforestation.", unsafe_allow_html=True)
    tab5.markdown(
        f"""You owe nature <b>{tree_count}</b> tree{'s' if tree_count > 1 else ''} monthly.<br>
        <a href='https://www.tema.org.tr/en/homepage' target='_blank' class='button-17' role='button'>🌳 Proceed to offset 🌳</a>""",
        unsafe_allow_html=True
    )

    # 🌿 Gemini AI Personalized Tips
    if tab5.button("🌿 Get My Personalized Green Plan"):
        with st.spinner("Generating AI recommendations..."):
            tips = generate_personalized_tips(
                transport_emission, electricity_emission, diet_emission, waste_emission
            )
            tab5.markdown("### ✅ Your AI-Recommended Eco Tips")
            tab5.success(tips)
